...
| FFN 激活 | ReLU 两层 | SwiGLU 门控三矩阵 |
| **FFN 稠密还是稀疏** | **每个 token 过整个 FFN** | **大模型多为 MoE：每个 token 只过少数几个专家** |
| 位置编码 | 正弦绝对位置 | RoPE |
| KV head | MHA | GQA / MQA |
| **attention 怎么算出来** | **把 $T \times T$ 那张矩阵真的算出来存着** | **FlashAttention：分块算，那张矩阵从不落进显存** |

真正值得注意的是这张表里**没有**的东西：缩放点积注意力的公式、position-wise FFN 的存在、残差连接——这三件核心组件八年没有实质变化。

注意最后两行和前面几行性质不同：前面几行改的是**结构**，最后两行一个改的是**哪些参数参与计算**（MoE），一个改的是**同一个公式怎么落到硬件上**（FlashAttention）。后者尤其容易被误读成「换了个更强的 attention」——它算出来的结果和朴素实现逐位相同。
...
| 部分 | 计算量 | 序列长度翻 $k$ 倍 |
| --- | --- | --- |
| $QK^\top$ 与 $\text{weights} \times V$ | $O(T^2 d)$ | $\times k^2$ |
| QKVO 投影 + FFN | $O(T d^2)$ | $\times k$ |
| KV cache 显存 | $O(T)$ | $\times k$ |

代入数字才有体感：8k 提到 128k 是 $k=16$，三行分别涨 256 倍、16 倍、16 倍；提到 100 万 token 是 $k=128$，平方那行直接涨 16384 倍。**三行里只有平方项会失控**，另外两项老老实实跟着长度线性走。

`T` 远小于 `d` 时 FFN 主导，`T` 长到可比时 attention 的平方项接管。所谓「长上下文很贵」，贵的是这三件增长速度完全不同的事，谈的时候要分开说。

### 那张 $T \times T$ 的矩阵，其实不用真的存下来

上面按公式算的时候，中间有一张 `[B, h, T, T]` 的 `scores`。$T = 8192$ 时，单是这一张表每个头就有 6700 万个数——**它比参数还占地方，而且只用一次就丢掉**。

[FlashAttention](https://arxiv.org/abs/2205.14135)（Tri Dao、Daniel Y. Fu、Stefano Ermon、Atri Rudra、Christopher Ré，斯坦福大学与纽约州立大学布法罗分校，2022）的观察是：**瓶颈不在算，在搬**。显存（HBM）和计算单元旁边那块 SRAM 之间的带宽差着一个数量级，而朴素实现要把这张大矩阵写进显存再读回来。

做法叫**分块（tiling）**：把 Q、K、V 切成小块，一块一块地算局部的 softmax 加权和，用一个在线更新的技巧把各块结果合并成正确的全局结果。**那张完整的 $T \times T$ 矩阵从头到尾没有在显存里出现过。**

三点要分清：

- **它是精确的，不是近似。** 输出和朴素实现逐位相同——这和稀疏 attention、线性 attention 那类「用精度换速度」的方案是两回事。
- **省的是显存和搬运，不是浮点运算量。** $O(T^2 d)$ 那一项一次没少，只是不再为它付访存的钱。
- **它是实现层的优化。** 换上它不改变模型能力，只改变同一个模型能开多长的上下文、跑多快。

原文报告的加速在 GPT-2（序列长 1K）上是 3 倍，长序列上更明显。**这也是「为什么长上下文在工程上突然变得可行」的一大半答案**——公式里的平方项还在，但它不再意味着平方级的显存。

同一条思路的更一般形式叫**算子融合**：把几步连续的计算合并成一个 kernel，中间结果只在片上流转、不落显存。为什么这件事收益这么大，见 [[02-算力经常用不满：两种瓶颈]]。

### MoE：不是所有参数都参与每次计算

前面算参数量时默认「每个 token 都要过完整的 FFN」。**大模型现在多数不是这样了。**

**MoE（Mixture of Experts，专家混合）** 把一个大 FFN 换成很多个小 FFN（叫专家），外加一个路由器决定每个 token 走哪几个：

| | DeepSeek-V3 | Qwen3-235B-A22B |
| --- | --- | --- |
| 总参数 | 6710 亿 | 2350 亿 |
| **每 token 实际激活** | **370 亿** | **220 亿** |
| 每层专家数 | 1 个共享 + 256 个路由 | — |
| 每 token 激活几个 | 8 个路由专家 | — |

（DeepSeek-V3 那一列来自它的技术报告；Qwen3 的两个数就写在模型名字里——`235B-A22B` 的 A 就是 activated。）

**所以「参数量」这个词对 MoE 模型有两个值，谈的时候必须说清是哪个：**

- **总参数**决定**显存要多大**——权重全都得装进去，路由器随时可能点到任何一个专家
- **激活参数**决定**算得多快、多贵**——每个 token 实际只过这么多参数

DeepSeek-V3 是 671B 的显存开销配 37B 的计算开销。这就是 MoE 全部的卖点：**用显存换算力**。

它带来的工程后果一路传下去：显存测算要按总参数算（见 [[03-KV cache 管理与显存测算]]）、训练时多一个「专家并行」维度且通信不规则（见 [[04-多卡怎么连]]）、以及推理时的负载不均——路由不均匀会让某些专家所在的卡忙死、别的闲着。
...
- [ ] 有人说「上下文放长十倍」，能当场分开说出：attention 计算量涨一百倍，FFN 和 KV cache 各涨十倍
- [ ] 能说出为什么 K、V 可以缓存而 Q 不行
- [ ] 被问「为什么除 $\sqrt{d_k}$」，答的是梯度饱和，不是「为了归一化」
- [ ] 能解释 prefill compute-bound、decode memory-bound 的成因，而不是只背结论
- [ ] 知道 FlashAttention 是精确的实现层优化，省的是访存不是浮点运算
- [ ] 说到 MoE 模型的「参数量」会先分清是总参数还是激活参数，并说得出各自决定什么
...
**拿 MoE 的一个参数量估所有事。** 总参数决定显存、激活参数决定算力，两者能差十几倍。用总参数估延迟会离谱地悲观，用激活参数估显存会直接 OOM。

**把 FlashAttention 当成一种新的 attention。** 它算出来的结果和朴素实现逐位相同，改的只是怎么落到硬件上。看到「我们用了 FlashAttention 所以效果更好」这类说法，可以直接判断作者没搞清它在哪一层。
...
| 2022 | [FlashAttention](https://arxiv.org/abs/2205.14135) | Dao、Fu、Ermon、Rudra、Ré / 斯坦福、纽约州立大学布法罗分校 | 「那张 $T \times T$ 矩阵不用真的存下来」整节，含 GPT-2 上 3 倍加速那个数 |
| 2024 | [DeepSeek-V3 技术报告](https://arxiv.org/abs/2412.19437) | DeepSeek-AI | MoE 那节的 6710 亿 / 370 亿、1 个共享 ⊕ 256 个路由专家、每 token 激活 8 个 |
